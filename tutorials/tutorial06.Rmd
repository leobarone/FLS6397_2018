...
O arquivo de janeiro 2017 contém 1.6GB. É um arquivo grande, com 14 variáveis (colunas) e mais de 12 milhões de linhas. Arquivos grandes podem ser abertos no R usando a função _fread_, disponível no pacote _data.table_. 
...
Vamos agora baixar o arquivo de janeiro 2017 e abrir os dados com a função _fread_, que vimos em tutoriais passados, substituindo com a pasta relevante no seu computador.
...
Finalmente, vamos fazer o somatório dos valores sacado por UF em janeiro 2017:
...
```{r}
saques_amostra_201701 <- saques_amostra_201701 %>% 
  mutate(valor_dolar = valor_num / 3.2, valor10 = valor_num + 10)
```
